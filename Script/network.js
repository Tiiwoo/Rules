...
function getIP() {
  ...
  let info = [];
  if (!v4 && !v6) {
    info = ['Network may switch', 'Please manually refresh to obtain IP again'];
  } else {
  ...
  httpMethod
    ...
    .then((response) => {
      ...
      $done({
        ...
        icon: getSSID() ? 'wifi.circle' : 'personalhotspot.circle',
